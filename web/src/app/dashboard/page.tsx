...
function DashboardContent() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      let worksListQuery = supabase.from('obras').select('*').eq('status', 'ativa').order('created_at', { ascending: false }).limit(20);
      ...
      if (query) {
        ...
        // Nota: A lista de obras (worksListQuery) não é mais filtrada pela busca global
        // para que a tabela sempre mostre todas as obras ativas para seleção.
      }
      ...
      const works = (worksRaw || []).map(obra => {
        ...
        const progresso_esperado = Math.min(100, Math.round((daysElapsed / 60) * 100)); // Meta de 60 dias
        const status_prazo = obra.progresso_total < (progresso_esperado - 10) ? 'atrasada' : 'em_dia'; // 10% de tolerância
        return { ...obra, progresso_esperado, status_prazo };
      });

      // Calcular KPI de atrasadas com base na filtragem se houver busca, ou sobre todas as ativas (limitadas a 20)
      // Para ser exato em larga escala, precisaríamos de uma View. Mas para uso atual (algumas dezenas), isso funciona perfeitamente.
      let atrasadas = 0;
      if (worksRaw && worksRaw.length > 0) {
        // Se tem query, conta apenas entre as obras filtradas no worksListQuery localmente
  ...
  // Exibir feedback flutuante (Toast)
  ...
  const chartData = useMemo(() => {
    ...
    // Se clicou em uma obra na tabela, usa ela, senão pega a primeira
    const obra = selectedObraId ? allWorks.find(o => o.id === selectedObraId) || allWorks[0] : allWorks[0];
    const start = new Date(obra.data_inicio).getTime();
    const now = Date.now();
    const daysElapsed = Math.max(0, (now - start) / (1000 * 60 * 60 * 24));
    ...
    return Array.from({ length: totalSemanas }).map((_, i) => {
      ...
      if (weekNum <= weeksElapsed) {
        ...
        progressoReal = Math.round((obra.progresso_total / weeksElapsed) * weekNum);
      }
  ...
  return (
    <div className="space-y-8 pb-12 relative">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        {/* Contêiner de botões responsivos (Esticam no mobile, lado a lado no desktop) */}
        <div className="flex flex-col gap-3 sm:flex-row w-full sm:w-auto">
          ...
          <button 
            ...
            className="flex items-center justify-center rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition-all hover:bg-emerald-700 active:scale-95 w-full sm:w-auto cursor-pointer"
          >
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-slate-100 text-slate-500">
                ...
                <th className="pb-4 font-medium">Frequência Mensagens</th>
              ...
              {allWorks.length > 0 ? (
                allWorks.map((obra) => (
                  <tr 
                    ...
                    className={`group cursor-pointer transition-colors ${selectedObraId === obra.id ? 'bg-emerald-50/50' : 'hover:bg-slate-50/50'}`}
                  >
                    ...
                    <td className="py-4">
                      <div className="flex items-center gap-2">
                        <div className="h-1.5 w-16 rounded-full bg-slate-100">
                          <div className="h-full rounded-full bg-emerald-500" style={{ width: `${obra.progresso_total}%` }} />
                        </div>
                    ...
                    <td className="py-4 text-slate-500">Semanal (Seg, 08h)</td>
                    <td className="py-4 text-right">
                      ...
                      <div className="flex flex-col sm:flex-row justify-end items-center gap-2">
                        <button 
                          ...
                          className="flex items-center justify-center rounded-xl bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-700 hover:bg-emerald-100 active:scale-95 transition-all w-full sm:w-auto disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
                        >
                          <Send className="mr-1.5 h-3.5 w-3.5" />
                          {dispatchingId === obra.id ? 'Enviando...' : 'Forçar Disparo'}
                        </button>
                        
                        <button className="flex items-center justify-center rounded-xl border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-50 active:scale-95 transition-all w-full sm:w-auto cursor-pointer">
                          Ver Detalhes
                        </button>
                      </div>
                    </td>
          ...
          <div className="w-full max-w-md rounded-3xl border border-slate-100 bg-white p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-250">
            ...
            <div className="space-y-4">
              <div>
                ...
                <select 
                  ...
                  className="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm text-slate-800 focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500"
                >
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-3 justify-end">
              ...
              <button 
                ...
                className="w-full sm:w-auto rounded-xl bg-emerald-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-emerald-700 transition-colors active:scale-95 cursor-pointer"
              >
