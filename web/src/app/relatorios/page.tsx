  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button className="flex items-center rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-emerald-700">
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {['Produtividade', 'Atrasos', 'Geral'].map((tipo) => (
          <div key={tipo} className="p-6 rounded-2xl border border-slate-200 bg-white shadow-sm hover:shadow-md transition-shadow cursor-pointer group">
            <div className="h-12 w-12 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-emerald-50 group-hover:text-emerald-600 mb-4 transition-colors">
              <FileText size={24} />
            </div>
        ...
        <div className="divide-y divide-slate-100">
          {relatorios.map((rel) => (
            <div key={rel.id} className="flex items-center justify-between p-4 hover:bg-slate-50/50 transition-colors">
              ...
              <button className="p-2 rounded-lg text-slate-400 hover:bg-white hover:text-emerald-600 hover:shadow-sm">
                <Download size={20} />
              </button>
            </div>
          ))}
